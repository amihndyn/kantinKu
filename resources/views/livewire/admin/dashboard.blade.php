						...
						<div class="grid grid-cols-1 gap-4 sm:grid-cols-3 md:gap-6">
							...
							<div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
								<div class="flex h-12 w-12 items-center justify-center rounded-xl bg-gray-100 dark:bg-gray-800">
									<svg class="text-gray-800 dark:text-white/90" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-bar-chart-2">
							...
							<div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
								<div class="flex h-12 w-12 items-center justify-center rounded-xl bg-gray-100 dark:bg-gray-800">
									<svg class="text-gray-800 dark:text-white/90" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-shopping-cart">
					...
					<div class="col-span-12">
						...
						<div class="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
							<div class="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
								<div class="w-full">
									<h3 class="text-lg font-semibold text-gray-800 dark:text-white/90"> Monthly Statistics </h3>
									<p class="mt-1 text-gray-500 text-theme-sm dark:text-gray-400"> Visitors, Users, Products </p>
								</div>
							</div>
							<div class="max-w-full overflow-x-auto custom-scrollbar">
								<div id="chartStats" class="-ml-4 min-w-[700px] pl-2"></div>
							</div>
						</div>
						<!-- ====== Chart Three End -->
					</div>
	...
	<script>
		document.addEventListener('alpine:init', () => {
			Alpine.data('chartComponent', () => ({
				init() {
					const visitors = @json($visitors ?? array_fill(0, 12, 0));
					const users    = @json($users ?? array_fill(0, 12, 0));
					const products = @json($products ?? array_fill(0, 12, 0));
					...
					const options = {
						...
						fill: { gradient: { enabled: true, opacityFrom: 0.5, opacityTo: 0 } },
						xaxis: {
							type: "category",
					...
					const chart = new ApexCharts(document.querySelector("#chartStats"), options);
					chart.render();
				}
			}));
		});
