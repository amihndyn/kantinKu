<div>
                ...
                <div class="flex flex-col lg:flex-row gap-10">
                        ...
                        <div class="bg-white rounded-2xl p-6 shadow-xl border border-gray-200 sticky top-24">
                            ...
                            <div class="relative mb-6">
                                <input 
                                    ...
                                    class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0C2B4E] focus:outline-none"
                                >
                            ...
                            <div class="space-y-2">
                                <button 
                                    ...
                                    class="w-full text-left px-4 py-2 rounded-lg {{ !$selectedCategory ? 'bg-[#0C2B4E] text-white font-semibold shadow' : 'hover:bg-gray-100 text-gray-700' }} transition"
                                >
                                    Semua 
                                </button>
                                @foreach ($categories as $category)
                                    <button 
                                        ...
                                        class="w-full text-left px-4 py-2 rounded-lg {{ $selectedCategory == $category->id ? 'bg-[#0C2B4E] text-white font-semibold shadow' : 'hover:bg-gray-100 text-gray-700' }} transition font-medium"
                                    >
                                        {{ $category->name }} 
                                    </button>
                                @endforeach
                    ...
                    <div class="flex-1">
                        <div class="mb-6 text-gray-600"> 
                            Menampilkan <span class="font-bold text-[#0C2B4E]">{{ $products->count() }}</span> produk 
                        </div>
                            ...
                            @foreach ($products as $product)
                                ...
                                <div class="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl border border-gray-100 transition transform hover:-translate-y-2">
                                    <div class="relative h-56 overflow-hidden">
                                        <img 
                                            src="{{ $product->image_path ? asset('storage/' . $product->image_path) : 'https://source.unsplash.com/400x300/?food,' . Str::slug($product->name) }}" 
                                            class="object-cover w-full h-full hover:scale-110 transition-transform duration-300" 
                                            alt="{{ $product->name }}"
                                        />
                                        <span class="absolute top-3 left-3 bg-[#FFD700] text-[#0C2B4E] px-3 py-1 text-xs font-bold rounded-full shadow">
    ...
    @if($showDetail && $selectedProduct)
        <livewire:product.product-detail :product="$selectedProduct" />
    @endif
</div>