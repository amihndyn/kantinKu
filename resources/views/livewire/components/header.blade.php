<!-- ===== Header Start ===== -->
<header x-data="{ page: 'dashboard', sidebarToggle: false, 'loaded': true, 'darkMode': false, 'stickyMenu': false, 'scrollTop': false, 'menuToggle': false }" x-init="darkMode = JSON.parse(localStorage.getItem('darkMode')); $watch('darkMode', value => localStorage.setItem('darkMode', JSON.stringify(value)))" class="sticky top-0 z-99999 flex w-full border-gray-200 bg-white lg:border-b dark:border-gray-800 dark:bg-gray-900">
            ...
            <div class="relative" x-data="{ dropdownOpen: false }">
                <a class="flex items-center text-gray-700 dark:text-gray-400" href="#" @click.prevent="dropdownOpen = ! dropdownOpen">
                    <span class="mr-3 h-11 w-11 overflow-hidden rounded-full">
                        <div class="flex items-center justify-center w-11 h-11 rounded-full bg-brand-100">
                            <span class="text-xs font-semibold text-brand-500"> {{ $initials }} </span>
                        </div>
                    </span>
                    <span class="text-theme-sm mr-1 block font-medium"> {{ explode(' ', $name)[0] }} </span>
                ...
                <div x-show="dropdownOpen" class="shadow-theme-lg dark:bg-gray-dark absolute right-0 mt-[17px] flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 dark:border-gray-800">
                    <div>
                        ...
                        <span class="text-theme-xs mt-0.5 block text-gray-500 dark:text-gray-400"> {{ $email }} </span>
                    </div>
                    ...
                    <form class="w-full" method="POST" action={{ route('logout') }}>
                        @csrf
                        <button type="submit" class="group text-theme-sm mt-3 flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300 w-full">
